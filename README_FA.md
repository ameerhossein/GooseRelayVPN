...
نسخهٔ فعلی `Code.gs` فقط متادیتای نسخه/پروتکل را از طریق `doGet` برای بررسی pre-flight برمی‌گرداند. عمداً در هر `doPost` چیزی در `PropertiesService` نمی‌نویسد، چون این کار به هر درخواست تونل latency اضافه می‌کند. عدد `today=N` سمت کلاینت در خط `[stats]` همچنان تخمین مصرف quota است.
